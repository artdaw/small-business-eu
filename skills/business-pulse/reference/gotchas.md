...
## Gotcha: Exact Online returns unexpected state

**Why it matters:** Exact Online can return empty results, a sync-in-progress state, or an auth error without throwing a hard exception. Treating silence as "no data" causes the pulse to skip Cash entirely without telling the owner — they assume their business has no Exact Online data.

### ✗ Bad

```
Claude: [Exact Online returns empty response]
        → skips Cash section silently
        → pulse presents no cash data; owner assumes Exact Online isn't connected
```

The owner spends 10 minutes reconnecting Exact Online before realizing it was always connected — just in a transient state.

### ✓ Good

```
Claude: [Exact Online returns empty response or error]
        → Cash section header present with: "n/a — Exact Online unavailable (sync error or auth required)"
        → "Sources unavailable: Exact Online — returned empty response" in appendix
```
...
**Why it matters:** The skill's core value is doing the work without prompting. An owner who invoked the pulse already implicitly approved the data pull. Asking "should I check Exact Online?" or "can I read your emails?" defeats the purpose and erodes trust in the skill as an autonomous assistant.

### ✗ Bad

```
Owner: "catch me up on the business"
Claude: "Should I check Exact Online for your cash balance? And is it okay to look at your HubSpot pipeline?"
```
