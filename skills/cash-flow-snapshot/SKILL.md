...
description: >
  Reads AR/AP, historical cash timing, and known fixed costs from Exact Online,
  PayPal, Mollie, or SumUp — or a CSV upload — and produces a 30/60/90-day
  cash flow forecast with percentage-variance confidence bands and named risk
  flags. Delivers a chat summary and a downloadable XLSX. Currency defaults to
  EUR. Use when the user asks "forecast my cash flow," "will I make payroll,"
  mentions "runway," or says "cash crunch." Falls back to CSV upload when no
  connector is live.
compatibility: "Requires one or more of: Exact Online MCP, PayPal MCP, Mollie MCP, SumUp MCP, file upload (CSV fallback). Output uses xlsx skill."
---
...
and a downloadable XLSX workbook. All amounts in EUR unless the owner's Exact
Online account uses a different currency.
...
1. Exact Online — primary source for AR aging, AP, and fixed costs
2. PayPal — transaction history and settlement timing
3. Mollie — payment and settlement history (SEPA, iDEAL, Bancontact)
4. SumUp — POS sales and payout history
...
- VAT position: outstanding VAT liability due in forecast window (flag as a fixed outflow)
...
- For Mollie: note SEPA Direct Debit mandates — recurring collections have predictable timing
...
- **VAT deadline:** "VAT return due April 30 — estimated liability €4,100 based
  on Exact Online turnover. Included in 30-day outflows."
...
3. **Risks** — the flagged risks with euro impact and affected window.
