...
## Gotcha: Exact Online division not configured

Exact Online requires a division code in the MCP configuration. If it's missing or incorrect, the connector returns an auth or "division not found" error.

**Why it matters:** A dead-end error frustrates the user. Pre-flight validation prevents wasted time.

### ✗ Bad
User: "What should I post?"
Skill: [calls Exact Online] → Error: "Division not found"
User: [confused, doesn't know what to do next]

### ✓ Good
User: "What should I post?"
Skill: [calls Exact Online] → Error: "Division not found"
Skill: "Exact Online isn't connecting — this usually means the division code in .mcp.json needs to be set. Check your Exact Online division code (found in the URL when logged in) and update EXACTONLINE_DIVISION. Want to proceed with payment processor data while you fix this?"

---

## Gotcha: Mollie/PayPal rate-limiting on repeated calls

If you call settlement or transaction endpoints multiple times in rapid succession (e.g., looping through date ranges), Mollie or PayPal may rate-limit and return 429 errors.
...
### ✓ Good
Call list_transactions → 429 error → Wait 30 seconds → Retry → If still blocked, offer fallback: "PayPal is temporarily rate-limited. Would you like to switch to Mollie or SumUp, or continue with partial data?"
