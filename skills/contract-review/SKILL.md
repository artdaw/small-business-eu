...
1. **Get the contract** — Pull from one of three sources, in order of preference:
   - **Gmail / Outlook**: Search for recent emails with contract attachments (see `reference/gmail-fetch.md`)
   - **DocuSign**: Fetch the envelope by ID or search recent drafts awaiting signature (see `reference/docusign-fetch.md`). Note: if the owner uses **YouSign** (eIDAS-qualified, France), it does not yet have an official MCP — fetch the contract from email instead.
