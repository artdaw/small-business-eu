...
An owner sees €50K in payment processor settlements and thinks that's what they made. It isn't — it's what they collected.
...
### ✗ Bad
"Your Mollie revenue last quarter was €50,000."
...
Owners often have discounts, refunds, and promotions that reduce what they actually receive per transaction. Mollie/PayPal/SumUp settlement data reflects actual collected amounts, but Exact Online invoices may show list price.

**Why it matters:** If you compute margins using list price but payment processors show actual collections, your cost-per-unit math will look better than reality.

### ✗ Bad
Use invoice amounts from Exact Online for revenue and Mollie settlement for COGS → margin appears inflated.

### ✓ Good
Use Mollie/PayPal/SumUp settlement amounts as the revenue source (actual collected), and Exact Online for costs. Note any discrepancy between Exact Online invoice totals and payment processor totals — it's worth surfacing.
...
## Gotcha: Exact Online COGS not broken down by product/service

Many small businesses record COGS as a single GL account, not broken out by product. Exact Online may not return item-level cost data from the P&L report alone.

**Why it matters:** You can't compute per-product margins if COGS is lumped together.

### ✗ Bad
Pull Exact Online P&L → Get total COGS €22,000 → Try to divide across 12 products → Numbers are meaningless.

### ✓ Good
If Exact Online doesn't have item-level COGS, ask the owner: "Exact Online has your total costs but not a breakdown by product. Do you have a rough sense of what each item costs you to make or deliver? Even ballpark figures work." Proceed with owner-provided figures and flag the limitation in the output.
...
### ✓ Good
Call `list_transactions` → if 429, pause 30 seconds → retry once → if the retry succeeds, continue normally, but treat any *second* 429 in the same session as a signal to stop retrying. After a second rate-limit event (even if separated by a successful call), immediately surface the fallback: "PayPal is rate-limiting repeated calls in this session. I can switch to Mollie or SumUp for the revenue data, or you can upload a PayPal CSV export — either works. What would you prefer?" Do not attempt a third retry.
