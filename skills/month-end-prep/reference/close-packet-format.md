...
A formatted copy of the Exact Online P&L for the target month. Two-column layout:
**Category** and **Amount** (EUR).
...
Side-by-side comparison of Exact Online deposits vs. processor settlements.

Columns:
| Column | Source |
|---|---|
| Date (EO) | Exact Online deposit date |
| Amount (EO) | Exact Online deposit amount (EUR) |
| Processor | Mollie / PayPal / SumUp |
| Date (Processor) | Processor arrival date |
| Amount (Processor) | Processor net payout (EUR) |
| Delta | EO amount minus processor amount |
| Status | RECONCILED / MISSING_IN_EO / UNMATCHED_DEPOSIT / DATE_MISMATCH |

Color-code the Status column:
- RECONCILED → green fill
- DATE_MISMATCH → yellow fill
- MISSING_IN_EO or UNMATCHED_DEPOSIT → red fill
