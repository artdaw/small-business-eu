...
1. Pull Exact Online revenue by product/service for the last 90 days (EUR).
2. Pull COGS or direct costs per product from Exact Online (if categorized).
3. Pull Mollie, PayPal, and SumUp gross sales for the same products to cross-validate.
...
- Project revenue at current price × current volume (EUR)
...
If Exact Online is unreachable, stop — margin analysis requires Exact Online revenue and cost data. If payment processors (Mollie/PayPal/SumUp) are missing, run from Exact Online-only and note "Payment processors not connected — cross-validation skipped."

## Approval gates

- **Never recommend a specific price.** Provide data views only — pricing decisions belong to the owner.
- **Flag if COGS data is incomplete** (many Exact Online setups don't track per-product COGS) and note the gap.
- **Never update any prices in Exact Online or any connected system.**

## Output

Present the margin table (EUR), then the three scenario tables side-by-side. If a price increase scenario is being considered, append the customer messaging brief. End with: "Which scenario would you like to explore further?"
