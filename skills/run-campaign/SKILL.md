---
name: run-campaign
description: Runs an end-to-end marketing campaign — sales analysis, content brief, Canva assets, CRM send. Accepts optional lookback and channel arguments.
...
1. Pull sales data from Exact Online and Mollie/PayPal/SumUp for the lookback window (EUR).
...
5. Stage the scheduled send in HubSpot or Brevo (do NOT send — staging only). Use whichever CRM is connected; prefer HubSpot if both are connected.
...
1. Pull contacts from HubSpot, Brevo, or Pipedrive that match the campaign's target segment (from the approved brief).
...
4. Block calendar time for the call list or generate Calendly links.
5. Present both lists. Wait for explicit "send" before pushing the CRM campaign live.

## Approval gates (must hold)

- Never auto-progress between steps. Each handoff requires explicit owner approval.
- Never send the CRM campaign without the owner's "send" command in Step 3.
- If any connector is unreachable (Exact Online, Mollie/PayPal/SumUp, Canva, HubSpot/Brevo), stop, report which connector failed, and ask whether to retry or abort.

## Output

End the run with a one-paragraph recap: revenue dip identified, posts generated, segment size, calls booked. Link to the CRM campaign URL once sent.
