...
1. Pull Exact Online revenue by product/service category for the lookback period (EUR).
2. Pull Mollie, PayPal, and SumUp settlements for the same period to cross-validate and fill gaps.
3. Rank products by: total revenue (EUR), unit volume, and margin (if available in Exact Online).
...
1. Compare current period to same period in prior year (if Exact Online history available).
...
- Cross-reference with HubSpot, Brevo, or Pipedrive campaign activity for the period
...
If Exact Online is unreachable and no payment processor data is available, stop — sales analysis requires at least one revenue source. If only Mollie/PayPal/SumUp are connected (no Exact Online), run from those and note "Exact Online not connected — revenue data from payment processors only." If CRM (HubSpot/Brevo/Pipedrive) is missing, skip campaign cross-reference in the "why analysis" and note it.
...
Present the sales analysis (amounts in EUR), then the content brief. Ask the owner if they'd like to generate Canva assets for any of the planned posts.
